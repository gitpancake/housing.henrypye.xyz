...
const USER_COLORS: Record<number, { bg: string; text: string; border: string }> = {
  0: {
...
export function CalendarView({
  ...
  return (
    <div className="space-y-4">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      {dayViewings.map((v) => {
                        const colorIdx = userIndex.get(v.user.id) ?? 0
                        const colors = USER_COLORS[colorIdx] || USER_COLORS[0]
                        return (
                          <button
                            key={v.id}
                            className={`w-full text-left rounded px-1.5 py-0.5 text-[11px] leading-tight border ${colors.bg} ${colors.text} ${colors.border} hover:opacity-80 transition-opacity ${
                              v.status === "CANCELLED" ? "opacity-50 line-through" : ""
                            }`}
                            onClick={(e) => {
                              e.stopPropagation()
                              openEditDialog(v)
                          ...
                          >
                            <div className="font-medium truncate">
                              {format(new Date(v.scheduledAt), "h:mm a")}
                            </div>
                            <div className="truncate">{v.listing.title}</div>
                          </button>
                        )
                      })}
                    </div>
            ...
            <div className="min-w-[700px]">
              ...
              {weekHours.map((hour) => (
                <div key={hour} className="grid grid-cols-[60px_repeat(7,1fr)] border-b min-h-[60px]">
                  ...
                  {days.map((day, di) => {
                    const dayViewings = getViewingsForDay(day).filter((v) => {
                    ...
                    return (
                      ...
                      >
                        {dayViewings.map((v) => {
                          const colorIdx = userIndex.get(v.user.id) ?? 0
                          const colors = USER_COLORS[colorIdx] || USER_COLORS[0]
                          const area = getAreaFromAddress(v.listing.address)
                          return (
                            <button
                              key={v.id}
      ...
      {/* Upcoming viewings list (below calendar) */}
      {viewings.filter((v) => v.status === "SCHEDULED" && new Date(v.scheduledAt) >= startOfDay(new Date())).length > 0 && (
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <div className="space-y-3">
              {viewings
                .filter((v) => v.status === "SCHEDULED" && new Date(v.scheduledAt) >= startOfDay(new Date()))
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-xs text-muted-foreground mt-0.5">
                          <span>{format(new Date(v.scheduledAt), "EEE, MMM d 'at' h:mm a")}</span>
                          <span>30 min</span>
                          {area && (
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground mb-6">
              Schedule apartment viewings and track them on this shared calendar.
            </p>
            <Button onClick={() => openAddDialog()}>
              <Plus className="h-4 w-4 mr-2" />
