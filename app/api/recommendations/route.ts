  ...
  if (fetchAll) {
    ...
    for (const user of users) {
      ...
      const currentHash = computePreferencesHash({
        ...
        customDesires: (user.preferences.customDesires as { label: string; enabled: boolean }[]) || [],
      })
      ...
      staleness[user.id] = !latestRec || latestRec.preferencesHash !== currentHash
    }
...
export async function POST() {
  ...
  try {
    await generateRecommendations(session.userId)
