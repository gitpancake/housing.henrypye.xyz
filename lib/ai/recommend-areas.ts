...
async function recommendAreas(
    ...
    priorities.push(
        `Budget: $${preferences.budgetMin}-$${preferences.budgetMax}/month CAD`,
    );
    priorities.push(
        `Bedrooms: ${preferences.bedroomsMin}-${preferences.bedroomsMax}`,
    );

    const togglePrefs = [
        {
            key: "naturalLight" as const,
            label: "Lots of natural light (newer buildings, south-facing units)",
        },
        {
            key: "outdoorsAccess" as const,
            label: "Outdoor access (balcony, patio, proximity to parks and trails)",
        },
        {
            key: "publicTransport" as const,
            label: "Excellent public transit (close to SkyTrain stations, frequent bus routes)",
        },
        {
            key: "quietNeighbourhood" as const,
            label: "Quiet, peaceful neighbourhood (residential, low traffic noise)",
        },
        {
            key: "laundryInUnit" as const,
            label: "In-unit laundry (common in newer buildings)",
        },
        {
            key: "parking" as const,
            label: "Parking availability (dedicated spot)",
        },
        {
            key: "modernFinishes" as const,
            label: "Modern finishes and updated appliances",
        },
        {
            key: "storageSpace" as const,
            label: "Good storage space (closets, storage locker)",
        },
        {
            key: "gymAmenities" as const,
            label: "Building amenities (gym, pool, common areas)",
        },
        {
            key: "petFriendly" as const,
            label: "Pet-friendly buildings and nearby parks/trails for pets",
        },
    ];
...
${
    excludedAreas.length > 0
        ? `EXCLUDED AREAS (do NOT recommend these — the user has already ruled them out):
${excludedAreas.map((a) => `- ${a}`).join("\n")}
...
export async function generateRecommendations(userId: string): Promise<void> {
    ...
    // Fetch dismissed areas for this user
